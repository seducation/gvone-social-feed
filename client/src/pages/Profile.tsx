  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <main className="mx-auto max-w-3xl px-5 py-8 sm:px-8">
      <section className="rounded-[1.75rem] bg-[#17171d] p-6 text-white shadow-[0_16px_42px_rgba(24,24,32,.15)]">
        ...
        <div className="mt-6 flex flex-wrap gap-2"><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/70"><MessageCircleQuestion className="h-3.5 w-3.5 text-[#a7a1ff]" /> {questions.length} question{questions.length === 1 ? "" : "s"} asked</span><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/70"><CornerDownRight className="h-3.5 w-3.5 text-[#a7a1ff]" /> {answers.length} answer{answers.length === 1 ? "" : "s"} shared</span></div>
      </section>
      <section className="mt-8"><div className="mb-4"><h2 className="text-xl font-semibold tracking-[-.03em]">Your questions</h2><p className="mt-1 text-sm text-[#7d8794]">Question Threads you have started from RSS stories.</p></div><div className="space-y-3">{questions.length ? questions.map((thread) => { const article = articleByUrl.get(normalizeStoryUrl(thread.storyUrl)); const feed = article ? dashboard.data?.feeds.find((item) => item.id === article.feedId) : undefined; const sourceLabel = feed?.customTitle || feed?.title || new URL(thread.storyUrl).hostname; return <Link key={thread.id} href={`/pulse/${thread.discussionId}#thread-${thread.id}`} className="block rounded-[1.25rem] border border-[#e3e6ec] bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#c8c4ff]"><div className="text-xs font-semibold text-[#635bff]">{sourceLabel}</div><h3 className="mt-1 line-clamp-2 text-base font-semibold">{article?.title || "RSS story reference"}</h3><div className="mt-3 flex items-center gap-1.5 text-xs font-semibold text-[#8b7ade]"><MessageCircleQuestion className="h-3.5 w-3.5" /> Question Thread</div><p className="mt-2 line-clamp-3 text-sm leading-6 text-[#657080]">{thread.content}</p></Link>; }) : <div className="rounded-[1.5rem] border border-dashed border-[#d8dce4] bg-white/55 px-6 py-12 text-center"><MessageCircleQuestion className="mx-auto h-6 w-6 text-[#a9a2dc]" /><p className="mt-3 text-sm text-[#7d8794]">Questions you ask about RSS stories will appear here.</p></div>}</div></section>
      <section className="mt-10"><div className="mb-4"><h2 className="text-xl font-semibold tracking-[-.03em]">Your answers</h2><p className="mt-1 text-sm text-[#7d8794]">Quote answers you have shared with a clear return path to the original question.</p></div><div className="space-y-3">{answers.length ? answers.map((answer) => { const article = articleByUrl.get(normalizeStoryUrl(answer.storyUrl)); const feed = article ? dashboard.data?.feeds.find((item) => item.id === article.feedId) : undefined; const sourceLabel = feed?.customTitle || feed?.title || new URL(answer.storyUrl).hostname; return <Link key={answer.id} href={`/pulse/${answer.discussionId}#thread-${answer.parentPostId}`} className="block rounded-[1.25rem] border border-[#e3e6ec] bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#c8c4ff]"><div className="text-xs font-semibold text-[#635bff]">{sourceLabel}</div><h3 className="mt-1 line-clamp-2 text-base font-semibold">{article?.title || "RSS story reference"}</h3><div className="mt-3 rounded-xl border border-[#e5e2ff] bg-[#faf9ff] px-3 py-2 text-xs text-[#697280]"><div className="flex items-center gap-1.5 font-semibold text-[#635bff]"><Quote className="h-3.5 w-3.5" /> Answering @{answer.quotedUsername || answer.parentUsername || "member"}’s question</div><p className="mt-1 line-clamp-2 leading-5">{answer.quotedContent || answer.parentContent || "Open the question"}</p></div><div className="mt-3 flex items-center gap-1.5 text-xs font-semibold text-[#8b7ade]"><CornerDownRight className="h-3.5 w-3.5" /> Quote answer · Open question</div><p className="mt-2 line-clamp-3 text-sm leading-6 text-[#657080]">{answer.content}</p></Link>; }) : <div className="rounded-[1.5rem] border border-dashed border-[#d8dce4] bg-white/55 px-6 py-12 text-center"><CornerDownRight className="mx-auto h-6 w-6 text-[#a9a2dc]" /><p className="mt-3 text-sm text-[#7d8794]">Answers you share will appear here with their original question.</p></div>}</div></section>
    </main>
  </div>;
}

  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[270px_minmax(0,680px)_290px]">
      ...
      <main className="min-w-0 px-5 py-8 sm:px-8">
        <div className="mb-7 flex items-end justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.16em] text-[#9ba2ae]"><span className="h-2 w-2 rounded-full bg-[#24c1b0]" /> Community feed</div><div className="flex flex-wrap items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-2xl ${channelTint(activeChannel?.kind ?? "all")}`}><ChannelIcon kind={activeChannel?.kind ?? "all"} className="h-5 w-5" /></span><h1 className="text-3xl font-semibold tracking-[-.045em]">{activeLabel}</h1></div><p className="mt-3 text-sm text-[#8a929f]">{activeDescription} {sourceCount ? `${sourceCount} active source${sourceCount === 1 ? "" : "s"} · newest first` : ""}</p></div><div className="flex shrink-0 flex-wrap items-center justify-end gap-2">{selectedSourceFeed && <button type="button" onClick={() => setSelectedSourceFeedId(null)} className="rounded-full border border-[#e1e4ea] bg-white px-3.5 py-2 text-xs font-semibold text-[#68707d] transition hover:border-[#635bff] hover:text-[#635bff]">Back to channel</button>}{channelCommunityHostname && <Link href={`/community/${encodeURIComponent(channelCommunityHostname)}`} aria-label={`Visit ${channelCommunityHostname} community`} className="inline-flex items-center gap-2 rounded-full border border-[#dcd9ff] bg-[#f4f3ff] px-3.5 py-2 text-xs font-semibold text-[#635bff] transition hover:border-[#635bff]"><Globe2 className="h-3.5 w-3.5" /> Visit community</Link>}<button type="button" onClick={() => activeGroup ? refreshGroup.mutate({ id: activeGroup }) : refreshAllFeeds.mutate()} disabled={refreshAllFeeds.isPending || refreshGroup.isPending} className="inline-flex items-center gap-2 rounded-full border border-[#e1e4ea] bg-white px-3.5 py-2 text-xs font-semibold text-[#68707d] transition hover:border-[#635bff] hover:text-[#635bff] disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${refreshAllFeeds.isPending || refreshGroup.isPending ? "animate-spin" : ""}`} /> Refresh</button></div></div>

...
function FeedVideo({ url, mimeType, poster, className }: { url: string | null; mimeType?: string | null; poster?: string | null; className: string }) {
  if (!url) return null;
  if (mimeType === "text/html") return <iframe title="Embedded feed video" src={url} className={className} allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen />;
  return <video controls playsInline preload="metadata" className={className} src={url} poster={poster ?? undefined}><track kind="captions" /></video>;
}

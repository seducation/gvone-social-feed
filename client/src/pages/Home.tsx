...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <section data-source-bar data-compact={isSourceBarCompact} className={`sticky top-[76px] z-20 border-b border-[#e6e8ed] bg-[#f7f8fa]/95 px-4 backdrop-blur-xl transition-[padding] duration-200 ease-out sm:px-8 ${isSourceBarCompact ? "py-1.5" : "py-2.5"}`}>
      <div className="mx-auto flex max-w-[1440px] items-center gap-2 py-0.5">
        ...
        <div className="flex shrink-0 items-center gap-2 border-l border-[#e0e3e9] pl-2">
          <button type="button" onClick={() => setShowCommunityComposer(true)} aria-label="Create community post" title="Create community post" className={`inline-flex shrink-0 items-center justify-center rounded-xl border border-[#dcd9ff] bg-[#f4f3ff] font-semibold text-[#635bff] transition hover:border-[#635bff] ${isSourceBarCompact ? "h-9 w-9" : "h-9 w-9"}`}><Plus className="h-4 w-4" /></button>
    ...
    <CommunityPostComposer open={showCommunityComposer} onOpenChange={setShowCommunityComposer} communities={providerCommunities.data ?? []} onPublished={(providerHostname) => { window.location.href = `/community/${encodeURIComponent(providerHostname)}`; }} />

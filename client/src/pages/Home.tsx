...
import { pauseEmbeddedShort, syncShortPlayback } from "@/lib/shortsPlayback";
...
function FeedVideo({ url, mimeType, poster, className, videoRef, iframeRef, muted = false, autoPlay = false }: { url: string | null; mimeType?: string | null; poster?: string | null; className: string; videoRef?: (node: HTMLVideoElement | null) => void; iframeRef?: (node: HTMLIFrameElement | null) => void; muted?: boolean; autoPlay?: boolean }) {
  if (!url) return null;
  if (mimeType === "text/html") {
    ...
    try {
      ...
      if (/(^|\.)youtube\.com$/i.test(embedded.hostname)) {
        ...
        embedded.searchParams.set("mute", "1");
  ...
  return <video ref={videoRef} controls playsInline muted={muted} preload="metadata" className={className} src={url} poster={poster ?? undefined}><track kind="captions" /></video>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    syncShortPlayback(shortVideoRefs.current, activeShortId);
    shortEmbedRefs.current.forEach((iframe, id) => {
      if (id !== activeShortId) pauseEmbeddedShort(iframe);
    });

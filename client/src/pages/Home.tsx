...
import { applySourceTabOrder, buildSourceChannels, filterArticlesForSourceChannel, moveEditableSourceTab, sourceHostname, type SourceChannelKey, type SourceChannelKind } from "@/lib/sourceCategories";
...
export default function Home() {
  ...
  const [selectedSourceFeedId, setSelectedSourceFeedId] = useState<number | null>(null);
  ...
  const visibleArticles = useMemo(() => selectedSourceFeedId ? (allArticles.data ?? []).filter((article) => article.feedId === selectedSourceFeedId) : activeGroup ? baseArticles : filterArticlesForSourceChannel(baseArticles, activeChannel), [selectedSourceFeedId, allArticles.data, activeGroup, activeChannel, baseArticles]);
  ...
  const selectedSourceFeed = selectedSourceFeedId ? feeds.find((feed) => feed.id === selectedSourceFeedId) : undefined;
  const channelCommunityHostname = selectedSourceFeed ? sourceHostname(selectedSourceFeed.url) : !activeGroup && activeChannel?.kind !== "all" && categoryFeeds[0] ? sourceHostname(categoryFeeds[0].url) : null;
  const communityHref = channelCommunityHostname ? `/community/${encodeURIComponent(channelCommunityHostname)}` : !activeGroup && activeChannel?.kind === "all" ? "/communities" : null;
  const communityActionLabel = channelCommunityHostname ? "Visit community" : "Visit all communities";
  const activeLabel = selectedSourceFeed ? selectedSourceFeed.customTitle || selectedSourceFeed.title : activeGroup ? groups.find((group) => group.id === activeGroup)?.name ?? "Collection" : activeChannel?.label ?? "All signals";
  const activeDescription = selectedSourceFeed ? "Stories from this private RSS source." : activeGroup ? "A focused collection from your private signal." : activeChannel?.description ?? "Every source in your private library.";
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    <header className="sticky top-0 z-30 flex h-[76px] items-center border-b border-[#e6e8ed] bg-[#f7f8fa]/90 px-5 backdrop-blur-xl lg:px-8">
      ...
      <div className="ml-auto flex items-center gap-2"><a href="/chat" className="hidden items-center gap-2 rounded-full border border-[#e1e4ea] bg-white px-3.5 py-2.5 text-sm font-semibold text-[#596270] transition hover:border-[#635bff] hover:text-[#635bff] sm:inline-flex"><Sparkles className="h-4 w-4" /> Chat</a><button type="button" onClick={() => setShowCommunityComposer(true)} aria-label="Create community post" title="Create provider community post" className="grid h-10 w-10 place-items-center rounded-full border border-[#dcd9ff] bg-[#f4f3ff] text-[#635bff] transition hover:border-[#635bff] hover:bg-[#eeedff]"><Plus className="h-4 w-4" /></button><Link href="/topics" aria-label="Open Topics" className="inline-flex items-center gap-1.5 rounded-full border border-[#dcd9ff] bg-[#f4f3ff] px-3 py-2.5 text-sm font-semibold text-[#635bff] transition hover:border-[#635bff]"><Hash className="h-4 w-4" /><span className="hidden sm:inline">Topics</span></Link><button onClick={() => setShowAdd(true)} className="inline-flex items-center gap-2 rounded-full bg-[#635bff] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_7px_18px_rgba(99,91,255,.2)] transition hover:-translate-y-0.5"><Plus className="h-4 w-4" /> <span className="hidden sm:inline">Add feed</span></button><button onClick={() => auth.logout()} aria-label="Sign out" className="grid h-10 w-10 place-items-center rounded-full border border-[#e1e4ea] bg-white text-[#727b89] transition hover:text-[#14161a]"><LogOut className="h-4 w-4" /></button></div>
    </header>
    ...
    <section data-source-bar data-compact={isSourceBarCompact} className={`sticky top-[76px] z-20 border-b border-[#e6e8ed] bg-[#f7f8fa]/95 px-4 backdrop-blur-xl transition-[padding] duration-200 ease-out sm:px-8 ${isSourceBarCompact ? "py-1.5" : "py-2.5"}`}>
      <div className="mx-auto flex max-w-[1440px] items-center gap-2 py-0.5">
        ...
        <div className="flex shrink-0 items-center gap-2 border-l border-[#e0e3e9] pl-2">
          <Link href="/topics/feed" aria-label="Open all topic posts" title="All topic community posts" className={`inline-flex shrink-0 items-center gap-2 rounded-xl border border-[#dcd9ff] bg-[#f4f3ff] px-3 font-semibold text-[#635bff] transition hover:border-[#635bff] ${isSourceBarCompact ? "h-9 w-9 justify-center" : "h-9 text-xs"}`}><Hash className="h-4 w-4" /><span className="hidden md:inline"># Topics</span></Link>
    ...
    <div className="mx-auto grid max-w-[1500px] lg:grid-cols-[270px_minmax(0,680px)_290px]">
      <aside className="hidden min-h-[calc(100vh-151px)] border-r border-[#e6e8ed] bg-[#17171d] px-4 py-5 text-white lg:block">
        ...
        <div className="mt-2 space-y-1">{categoryFeeds.length ? categoryFeeds.map((feed) => <div key={feed.id} className="group flex items-center gap-2 rounded-xl px-3 py-2.5 transition hover:bg-white/7"><img src={favicon(feed.faviconUrl)} alt="" className="h-5 w-5 rounded-md" onError={(event) => { event.currentTarget.src = favicon(null); }} />{feed.customTitle ? <span data-private-label className="min-w-0 max-w-full truncate rounded-full border border-[#9b93ff]/45 bg-[#8f88ff]/15 px-2 py-0.5 text-[11px] font-semibold text-[#d4d1ff]">Private · {feed.customTitle}</span> : <span className="min-w-0 flex-1 truncate text-sm text-white/60">{feed.title}</span>}<button type="button" onClick={() => refreshFeed.mutate({ id: feed.id })} aria-label={`Refresh ${feed.customTitle || feed.title}`} className="hidden text-white/40 hover:text-white group-hover:block"><RefreshCw className="h-3.5 w-3.5" /></button></div>) : <p className="px-3 py-2 text-xs leading-5 text-white/35">No sources in this channel yet.</p>}</div>
      </aside>

      <main className="min-w-0 px-5 py-8 sm:px-8">
        <div className="mb-6 flex flex-col gap-4 sm:mb-7 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.16em] text-[#9ba2ae]"><span className="h-2 w-2 rounded-full bg-[#24c1b0]" /> Community feed</div><div className="flex flex-wrap items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-2xl ${channelTint(activeChannel?.kind ?? "all")}`}><ChannelIcon kind={activeChannel?.kind ?? "all"} className="h-5 w-5" /></span><h1 className="text-3xl font-semibold tracking-[-.045em]">{activeLabel}</h1></div><p className="mt-3 text-sm text-[#8a929f]">{activeDescription} {sourceCount ? `${sourceCount} active source${sourceCount === 1 ? "" : "s"} · newest first` : ""}</p></div><div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:justify-end">{selectedSourceFeed && <button type="button" onClick={() => setSelectedSourceFeedId(null)} className="rounded-full border border-[#e1e4ea] bg-white px-3.5 py-2 text-xs font-semibold text-[#68707d] transition hover:border-[#635bff] hover:text-[#635bff]">Back to channel</button>}{communityHref && <Link href={communityHref} aria-label={channelCommunityHostname ? `Visit ${channelCommunityHostname} community` : "Visit all communities"} className="inline-flex items-center gap-2 rounded-full border border-[#dcd9ff] bg-[#f4f3ff] px-3.5 py-2 text-xs font-semibold text-[#635bff] transition hover:border-[#635bff]"><Globe2 className="h-3.5 w-3.5" /> {communityActionLabel}</Link>}<button type="button" onClick={() => activeGroup ? refreshGroup.mutate({ id: activeGroup }) : refreshAllFeeds.mutate()} disabled={refreshAllFeeds.isPending || refreshGroup.isPending} className="inline-flex items-center gap-2 rounded-full border border-[#e1e4ea] bg-white px-3.5 py-2 text-xs font-semibold text-[#68707d] transition hover:border-[#635bff] hover:text-[#635bff] disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${refreshAllFeeds.isPending || refreshGroup.isPending ? "animate-spin" : ""}`} /> Refresh</button></div></div>
        <div className="mb-6 rounded-[1.5rem] border border-[#e7e9ee] bg-white p-4 shadow-[0_8px_25px_rgba(24,31,45,.03)]"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-sm font-semibold"><Megaphone className="h-4 w-4 text-[#635bff]" /> {activeChannel?.kind === "youtube" ? "Channel roll call" : activeChannel?.kind === "reddit" ? "Community roll call" : "Source roll call"}</div><span className="rounded-full bg-[#f3f2ff] px-2.5 py-1 text-xs font-semibold text-[#635bff]">{categoryFeeds.length} joined</span></div><div className="mt-4 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{categoryFeeds.map((feed) => <button key={feed.id} type="button" onClick={() => { setActiveGroup(null); setSelectedSourceFeedId(feed.id); }} aria-label={`Open ${feed.customTitle || feed.title} source`} className={`flex min-w-[160px] items-center gap-2 rounded-xl px-3 py-2 text-left transition ${selectedSourceFeedId === feed.id ? "bg-[#eceaff] ring-1 ring-[#a7a1ff]" : "bg-[#f7f8fa] hover:bg-[#f1f2f5]"}`}><img src={favicon(feed.faviconUrl)} alt="" className="h-6 w-6 rounded-lg" onError={(event) => { event.currentTarget.src = favicon(null); }} /><div className="min-w-0">{feed.customTitle ? <span data-private-label className="inline-flex max-w-full truncate rounded-full border border-[#d7d1ff] bg-[#f0eeff] px-2 py-0.5 text-[10px] font-bold text-[#635bff]">Private · {feed.customTitle}</span> : <div className="truncate text-xs font-semibold text-[#596270]">{feed.title}</div>}</div></button>)}{!categoryFeeds.length && <span className="text-sm text-[#8a929f]">Add a feed to populate this private channel.</span>}</div></div>
        {visibleArticles.length === 0 ? <div className="rounded-[1.5rem] border border-dashed border-[#d9dde5] bg-white/50 p-12 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#eeedff] text-[#635bff]"><Layers3 className="h-6 w-6" /></div><h2 className="mt-5 text-xl font-semibold">No stories in this channel yet</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-[#8a929f]">Add a matching source or choose another community channel to continue reading.</p><button type="button" onClick={() => setShowAdd(true)} className="mt-6 rounded-full bg-[#14161a] px-5 py-2.5 text-sm font-semibold text-white">Add source</button></div> : <div className="space-y-4">{visibleArticles.map((article) => { const feed = feeds.find((item) => item.id === article.feedId); const providerHostname = feed ? sourceHostname(feed.url) : "unknown-source"; return <article key={article.id} className="group overflow-hidden rounded-[1.35rem] border border-[#e7e9ee] bg-white shadow-[0_8px_25px_rgba(24,31,45,.03)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_38px_rgba(24,31,45,.08)]"><div className="p-5 sm:p-6"><div className="mb-4 flex items-center gap-3"><img src={favicon(feed?.faviconUrl ?? null)} alt="" className="h-8 w-8 rounded-xl" onError={(event) => { event.currentTarget.src = favicon(null); }} /><div className="min-w-0">{feed?.customTitle && <span data-private-label className="inline-flex max-w-full truncate rounded-full border border-[#d7d1ff] bg-[#f0eeff] px-2 py-0.5 text-[10px] font-bold text-[#635bff]">Private · {feed.customTitle}</span>}<Link href={`/community/${encodeURIComponent(providerHostname)}`} className="block truncate text-xs font-semibold text-[#635bff] hover:text-[#4c42c7]">{providerHostname}</Link><div className="mt-0.5 text-xs text-[#a0a7b2]">{formatDate(article.publishedAt)}</div></div><button type="button" aria-label="Bookmark story" className="ml-auto rounded-lg p-2 text-[#b2b8c2] transition hover:bg-[#f4f5f8] hover:text-[#635bff]"><Bookmark className="h-4 w-4" /></button><button type="button" aria-label="More story options" className="rounded-lg p-2 text-[#b2b8c2] transition hover:bg-[#f4f5f8] hover:text-[#635bff]"><MoreHorizontal className="h-4 w-4" /></button></div><a href={article.link} target="_blank" rel="noreferrer" className="block"><h2 className="text-xl font-semibold leading-snug tracking-[-.025em] transition group-hover:text-[#635bff]">{article.title}</h2>{article.description && <p className="mt-3 text-sm leading-6 text-[#68707d]">{article.description}</p>}{article.thumbnailUrl && <img src={article.thumbnailUrl} alt="" className="mt-5 max-h-72 w-full rounded-2xl object-cover" loading="lazy" />}</a>{article.videoUrl && <div className="mt-5 overflow-hidden rounded-2xl bg-black"><FeedVideo url={article.videoUrl} mimeType={article.videoMimeType} poster={article.thumbnailUrl} className="max-h-96 w-full" /><div className="flex items-center gap-2 px-4 py-3 text-xs text-white/70"><Video className="h-3.5 w-3.5" /> Inline video from this feed</div></div>}<div className="mt-5 flex items-center justify-between border-t border-[#f0f1f4] pt-4"><a href={article.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-medium text-[#a0a7b2] hover:text-[#635bff]">Open original story <ArrowUpRight className="h-4 w-4" /></a></div></div></article>; })}</div>}
      </main>

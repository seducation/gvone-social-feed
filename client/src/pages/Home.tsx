  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <section data-source-bar data-compact={isSourceBarCompact} className={`sticky top-[76px] z-20 border-b border-[#e6e8ed] bg-[#f7f8fa]/95 px-4 backdrop-blur-xl transition-[padding] duration-200 ease-out sm:px-8 ${isSourceBarCompact ? "py-1.5" : "py-2.5"}`}>
      <div className="mx-auto flex max-w-[1440px] items-center gap-2 py-0.5">
        ...
        <div className="flex shrink-0 items-center gap-1 border-l border-[#e0e3e9] pl-2 sm:gap-2">
          <button type="button" onClick={() => setShowShorts(true)} aria-label="Open Shorts" className={`inline-flex shrink-0 items-center gap-2 rounded-xl bg-[#14161a] font-semibold text-white transition hover:bg-[#34343d] ${isSourceBarCompact ? "h-9 w-9 justify-center" : "px-3 py-2 text-xs"}`}><Video className="h-4 w-4" /><span className="hidden md:inline">Shorts</span>{!isSourceBarCompact && <span className="rounded-full bg-white/12 px-1.5 py-0.5 text-[10px] text-white/75">{videoArticles.length}</span>}</button>

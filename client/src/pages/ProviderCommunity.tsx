...
export default function ProviderCommunity() {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <main className="mx-auto max-w-3xl px-5 py-8 sm:px-8"><div className="mb-7 rounded-[1.5rem] bg-[#17171d] p-6 text-white"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.16em] text-white/45"><Radio className="h-3.5 w-3.5 text-[#a7a1ff]" /> gvone community</div><h1 className="mt-4 text-3xl font-semibold tracking-[-.045em]">{community.data.community.providerHostname}</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/55">Posts from gvone members who have added this provider to their RSS library.</p><div className="mt-5 flex flex-wrap gap-2">{communities.data?.map((item) => <Link key={item.id} href={`/community/${encodeURIComponent(item.providerHostname)}`} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${item.providerHostname === community.data?.community.providerHostname ? "bg-[#a7a1ff] text-[#17171d]" : "bg-white/10 text-white/65 hover:bg-white/15"}`}>{item.providerHostname}</Link>)}</div></div>

...
export default function StoryPulse() {
  ...
  const [answerTarget, setAnswerTarget] = useState<{ id: number; displayName: string | null; content: string | null } | null>(null);
  ...
  return <div className="min-h-screen bg-[#f7f8fa] text-[#14161a]">
    ...
    <main className="mx-auto max-w-3xl px-5 py-8 sm:px-8">
      ...
      <section className="mt-8">
        <div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-semibold tracking-[-.03em]">Questions</h2><p className="mt-1 text-sm text-[#7d8794]">Ask about this RSS story, then answer a member’s question directly.</p></div><span className="rounded-full bg-[#eeedff] px-3 py-1.5 text-xs font-semibold text-[#635bff]">{reposts.length} question{reposts.length === 1 ? "" : "s"}</span></div>
        <div className="rounded-[1.5rem] border border-[#e3e6ec] bg-white p-4 shadow-[0_8px_25px_rgba(24,31,45,.03)]">
          <label className="sr-only" htmlFor="repost-note">Ask a question</label>
          <textarea id="repost-note" value={content} onChange={(event) => setContent(event.target.value)} maxLength={600} rows={3} placeholder="What would you like to understand about this story?" className="w-full resize-none bg-transparent px-2 py-2 text-sm outline-none placeholder:text-[#a1a8b4]" />
          <div className="mt-2 flex items-center justify-between border-t border-[#eef0f3] pt-3"><span className="text-xs text-[#9aa2ae]">{content.length}/600</span><button type="button" disabled={!content.trim() || repost.isPending} onClick={() => repost.mutate({ discussionId, content })} className="inline-flex items-center gap-2 rounded-full bg-[#635bff] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"><MessageCircleQuestion className="h-4 w-4" /> Ask question</button></div>
        </div>
        <div className="mt-4 space-y-3">
          {reposts.length ? reposts.map((item) => <article key={item.id} id={`thread-${item.id}`} className="scroll-mt-6 rounded-[1.25rem] border border-[#e5e8ed] bg-white p-5"><div className="flex gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#f0eaff] text-sm font-bold text-[#704ee5]">{initial(item.displayName)}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="truncate text-sm font-semibold">{item.displayName || "gvone member"}</span><span className="text-xs text-[#9aa2ae]">· {relativeTime(item.createdAt)}</span></div><div className="mt-1 inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide text-[#8b7ade]"><MessageCircleQuestion className="h-3 w-3" /> Question Thread</div>{item.content && <p className="mt-3 text-sm leading-6 text-[#4f5968]">{item.content}</p>}<div className="mt-4 flex items-center gap-3"><button type="button" onClick={() => { setAnswerTarget({ id: item.id, displayName: item.displayName, content: item.content }); setAnswerContent(""); }} className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#635bff] hover:text-[#4c42c7]"><Quote className="h-3.5 w-3.5" /> Answer</button><span className="text-xs text-[#9aa2ae]">{item.replies.length} answer{item.replies.length === 1 ? "" : "s"}</span></div>{answerTarget?.id === item.id && <div className="mt-4 rounded-2xl border border-[#dcd9ff] bg-[#f8f7ff] p-3"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-1.5 text-xs font-semibold text-[#635bff]"><Quote className="h-3.5 w-3.5" /> Answering {item.displayName || "gvone member"}’s question</div><p className="mt-1 line-clamp-2 text-xs leading-5 text-[#6d7584]">{item.content}</p></div><button type="button" onClick={() => setAnswerTarget(null)} aria-label="Cancel answer" className="rounded-lg p-1 text-[#8a929f] hover:bg-white hover:text-[#4f5968]"><X className="h-4 w-4" /></button></div><label className="sr-only" htmlFor={`answer-note-${item.id}`}>Your answer</label><textarea id={`answer-note-${item.id}`} value={answerContent} onChange={(event) => setAnswerContent(event.target.value)} maxLength={600} rows={3} placeholder="Share a helpful answer…" className="mt-3 w-full resize-none rounded-xl border border-[#e1dfff] bg-white px-3 py-2.5 text-sm outline-none placeholder:text-[#a1a8b4] focus:border-[#a7a1ff]" /><div className="mt-2 flex items-center justify-between"><span className="text-xs text-[#9aa2ae]">{answerContent.length}/600</span><button type="button" disabled={!answerContent.trim() || reply.isPending} onClick={() => reply.mutate({ discussionId, parentPostId: item.id, quotedPostId: item.id, content: answerContent })} className="inline-flex items-center gap-1.5 rounded-full bg-[#635bff] px-3.5 py-2 text-xs font-semibold text-white disabled:opacity-50"><CornerDownRight className="h-3.5 w-3.5" /> Post answer</button></div></div>}{item.replies.length > 0 && <section aria-label={`Answers to ${item.displayName || "gvone member"}’s question`} className="mt-5 space-y-3 border-l-2 border-[#eeedff] pl-4">{item.replies.map((answer) => <article key={answer.id} className="rounded-2xl bg-[#fafbfc] p-4"><a href={`#thread-${item.id}`} className="block rounded-xl border border-[#e5e2ff] bg-white px-3 py-2 text-xs text-[#697280] hover:border-[#c8c4ff]"><span className="flex items-center gap-1.5 font-semibold text-[#635bff]"><Quote className="h-3.5 w-3.5" /> In answer to {answer.quotedDisplayName || item.displayName || "gvone member"}’s question</span><span className="mt-1 block line-clamp-2 leading-5">{answer.quotedContent || item.content}</span></a><div className="mt-3 flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#f0eaff] text-xs font-bold text-[#704ee5]">{initial(answer.displayName)}</span><span className="text-sm font-semibold">{answer.displayName || "gvone member"}</span><span className="text-xs text-[#9aa2ae]">· {relativeTime(answer.createdAt)}</span></div>{answer.content && <p className="mt-2 text-sm leading-6 text-[#4f5968]">{answer.content}</p>}</article>)}</section>}</div></div></article>) : <div className="rounded-[1.5rem] border border-dashed border-[#d8dce4] bg-white/55 px-6 py-12 text-center"><MessageCircleQuestion className="mx-auto h-6 w-6 text-[#a9a2dc]" /><p className="mt-3 text-sm text-[#7d8794]">Be the first to ask about this RSS story.</p></div>}
        </div>
      </section>
    </main>
  </div>;
